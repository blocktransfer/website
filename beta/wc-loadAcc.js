async function loadWalletConnectAcc() {
  ...
  // get from stellar:pubnet
  if (accounts.length > 0) {
    const stellarAddress = accounts[0]; // The user's Stellar address
    
    // Now you can work with the Stellar address
    console.log(`Stellar Pubnet address: ${stellarAddress}`);
    
  }
  });
  walletConnect.on("disconnect", (error, payload) => {
    window.location.href = "https://www.blocktransfer.com/404?wc-con";
  });
  walletConnect.on("accountsChanged", (accounts) => {
    window.location.href = "https://www.blocktransfer.com/404?wc-mod";
  });
  walletConnect.on("chainChanged", (chainId) => {
    window.location.href = "https://www.blocktransfer.com/404?wc-xcc";
  });
